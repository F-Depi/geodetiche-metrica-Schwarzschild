...
The program can determine on a case by case basis (using the initial conditions
...
green lines and in the interval $[\hat r_{\rm min}, ~ \texttt{R\_MAX}]$ to find
...
\begin{figure}[h]
    ...
    \caption{Overlapping orbits with different time increments $h$.
    The simulation is repeated multiple times with the same initial conditions,
    $\hat \ell = 1.8$, $\mathcal E = -0.042$, $\hat r_0 = 2$, starting
    radial velocity directed outward.
    We only changed $h$, almost halving it every time, ranging from
    $h = \num{2e-3}$ to $h = \num{1e-5}$.
    In the main plot and in the zoomed-in sections $x$ and $y$ axes have always
    the same scale.
...
The Wikipedia article \textit{Precession of orbits} \cite{enwiki:1242536958}
...
Taking the inner turning point as an example, the precession can be computed 
with
...
It is important to note that the error is not given by the overshoot in itself,
...
We probably did not encounter this issue in previous simulations because the
...
that the error is consistently larger than the one of the outer turning
point.
